...
Public Class ctrlStation
    ...
    Private Sub ctrlStation_Disposed(sender As Object, e As EventArgs) Handles Me.Disposed
        cts.Cancel()
    End Sub
    ...
    Private Async Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        If TryCast(Me.ParentForm, frmViewItem) IsNot Nothing Then
            Using useInventory As New frmUseInventory(TryCast(Me.ParentForm, frmViewItem).parentCard.itemDetails, frmUseInventory.Action.Station)
                useInventory.ShowDialog()
                Await TryCast(Me.ParentForm, frmViewItem).LoadItem
                TryCast(Me.ParentForm, frmViewItem).SelectTab(frmViewItem.Tab.Station)
                btnRefresh_Search_Click(Nothing, Nothing)
            End Using
        End If
    End Sub
    ...
    Private Async Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        ...
        Dim stationDetails As New StationDetails
        With stationDetails
            .StationID = dgv.Item(colStationID.Index, dgv.CurrentRow.Index).Value
            ...
            .Location = dgv.Item(colLocation.Index, dgv.CurrentRow.Index).Value
            .Good = dgv.Item(colStationedGood.Index, dgv.CurrentRow.Index).Value
            .Damaged = dgv.Item(colStationedDamaged.Index, dgv.CurrentRow.Index).Value
        End With

        Using editStation As New frmEditStation(stationDetails)
    ...
    Private Async Sub LoadRecordList(itemID As Integer)
        ...
        Try
            Using con As New SqlConnection(CONNECTION_STRING)
                con.Open()
                Using com As New SqlCommand("SELECT " & IIf(numTop.Value = 0, "", "Top(" & numTop.Value & ")") & " * FROM vwStation WHERE ItemID = @a AND (" & SearchFilter("@b") & ")", con)
                    ...
                    Using reader As SqlDataReader = Await com.ExecuteReaderAsync(cts.Token)
                        While Await reader.ReadAsync(cts.Token)
                            dgv.Rows.Add({reader("StationID"),
                                          ...
                                          reader("Damaged"),
                                          reader("Location"),
                                          reader("StationedGood"),
                                          reader("StationedDamaged")})
                        End While
                    End Using
                End Using
            End Using
        ...
        Catch ex As Exception
            If Not ex.Message.Contains("Operation cancelled by user.") Then
                Notification.Show("Failed to load stationed inventory list.", Color.FromArgb(230, 50, 50))
            End If
        End Try
    ...
    Private Function SearchFilter(parameterVar As String) As String
        ...
        Dim defaultFilter As String = "StationID" & searchOperator & parameterVar & searchBinder &
                                      ...
                                      "Location" & searchOperator & parameterVar & searchBinder &
                                      "StationedGood" & searchOperator & parameterVar & searchBinder &
                                      "StationedDamaged" & searchOperator & parameterVar & searchBinder
        ...
        If cbtnFilter.Checked Then
            ...
            End If
            If Not cbtnStationID.Checked Then
                filter = filter.Replace("StationID" & searchOperator & parameterVar & searchBinder, "")
            End If
            If Not cbtnLocation.Checked Then
                filter = filter.Replace("Location" & searchOperator & parameterVar & searchBinder, "")
            End If
            If Not cbtnGood.Checked Then
                filter = filter.Replace("StationedGood" & searchOperator & parameterVar & searchBinder, "")
            End If
            If Not cbtnDamaged.Checked Then
                filter = filter.Replace("StationedDamaged" & searchOperator & parameterVar & searchBinder, "")
            End If
        End If
