    ...
    Public Async Function LoadRecord(cancellationToken As Threading.CancellationToken) As Threading.Tasks.Task
        ...
        Try
            Using con As New SqlConnection(CONNECTION_STRING)
                con.Open()
                Using com As New SqlCommand("SELECT * FROM vwStation WHERE StationID = @a", con)
                    com.Parameters.AddWithValue("@a", StationID)
                    ...
                    Using reader As SqlDataReader = Await com.ExecuteReaderAsync(cancellationToken)
                        If reader.Read Then
                            ...
                            Good = reader("StationedGood")
                            Damaged = reader("StationedDamaged")
