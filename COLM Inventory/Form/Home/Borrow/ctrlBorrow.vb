...
Public Class ctrlBorrow
    ...
    Private Sub ctrlBorrow_Disposed(sender As Object, e As EventArgs) Handles Me.Disposed
        cts.Cancel()
    End Sub
    ...
    Private Async Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        If TryCast(Me.ParentForm, frmViewItem) IsNot Nothing Then
            Using useInventory As New frmUseInventory(TryCast(Me.ParentForm, frmViewItem).parentCard.itemDetails, frmUseInventory.Action.Borrow)
                useInventory.ShowDialog()
                Await TryCast(Me.ParentForm, frmViewItem).LoadItem
                TryCast(Me.ParentForm, frmViewItem).SelectTab(frmViewItem.Tab.Borrow)
                btnRefresh_Search_Click(Nothing, Nothing)
            End Using
        End If
    End Sub
    ...
    Private Async Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        ...
        Dim borrowDetails As New BorrowDetails
        With borrowDetails
            .BorrowID = dgv.Item(colBorrowID.Index, dgv.CurrentRow.Index).Value
            ...
            .Good = dgv.Item(colBorrowedGood.Index, dgv.CurrentRow.Index).Value
            .Damaged = dgv.Item(colBorrowedDamaged.Index, dgv.CurrentRow.Index).Value
        End With

        Using editBorrow As New frmEditBorrow(borrowDetails)
            editBorrow.ShowDialog()
            Await TryCast(Me.ParentForm, frmViewItem).LoadItem
            TryCast(Me.ParentForm, frmViewItem).SelectTab(frmViewItem.Tab.Borrow)
            btnRefresh_Search_Click(Nothing, Nothing)
        End Using
    ...
    Private Async Sub LoadRecordList(itemID As Integer)
        ...
        Try
            Using con As New SqlConnection(CONNECTION_STRING)
                con.Open()
                Using com As New SqlCommand("SELECT " & IIf(numTop.Value = 0, "", "Top(" & numTop.Value & ")") & " * FROM vwBorrow WHERE ItemID = @a AND (" & SearchFilter("@b") & ")", con)
        ...
        Catch ex As Exception
            If Not ex.Message.Contains("Operation cancelled by user.") Then
                Notification.Show("Failed to load borrowed inventory list.", Color.FromArgb(230, 50, 50))
            End If
        End Try
    ...
    Private Function SearchFilter(parameterVar As String) As String
        ...
        Dim defaultFilter As String = "BorrowID" & searchOperator & parameterVar & searchBinder &
                                      ...
                                      "BorrowedGood" & searchOperator & parameterVar & searchBinder &
                                      "BorrowedDamaged" & searchOperator & parameterVar & searchBinder
        ...
        If cbtnFilter.Checked Then
            ...
            End If
            If Not cbtnBorrowID.Checked Then
                filter = filter.Replace("BorrowID" & searchOperator & parameterVar & searchBinder, "")
            End If
            If Not cbtnGood.Checked Then
                filter = filter.Replace("BorrowedGood" & searchOperator & parameterVar & searchBinder, "")
            End If
            If Not cbtnDamaged.Checked Then
                filter = filter.Replace("BorrowedDamaged" & searchOperator & parameterVar & searchBinder, "")
            End If
        End If
